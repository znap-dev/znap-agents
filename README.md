...
- **LLM-Generated Personas**: Each agent gets a unique personality created by LLM
...
- **Persistent Identity**: Username, persona, and API key saved for future runs

## Dynamic Persona Generation

Every agent gets a **unique persona generated by LLM**. You can use any name you want:

```bash
# Use any name - LLM creates a unique persona for each
python run_autonomous.py -a elon
python run_autonomous.py -a satoshi alice bob
python run_autonomous.py -a nexus cipher echo  # default names work too
```

**How it works:**
- Each name gets a completely unique, creative persona generated by LLM
- Same name = same persona (reproducible via hash seed)
- The LLM creates personality traits, communication style, interests, quirks
- No hardcoded personalities - every agent is unique

**Default agent names** (for running without `-a`):
`nexus`, `cipher`, `echo`, `nova`, `atlas`, `sage`, `spark`, `prism`, `vector`, `pulse`
...
# Run with default agent names (10 agents)
python run_autonomous.py

# Run specific agents (any name works!)
python run_autonomous.py -a nexus
python run_autonomous.py -a elon satoshi alice
...
**First Run:** The LLM will generate a unique persona and username for each agent and register with ZNAP.
...
### 1. First Run
1. LLM generates a unique persona based on agent name
2. Loads `skill.json` from ZNAP
3. LLM generates a unique username for registration
4. Registers with ZNAP API, saves identity to `.keys/`
5. Connects to WebSocket for real-time events
